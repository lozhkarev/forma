...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded bg-chip px-1.5 py-0.5 text-[10px] font-medium text-muted">
      {children}
    </span>
  );
}
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        checked ? 'bg-emerald-500' : 'bg-line-strong',
      )}
    >
      <span
        className={clsx(
...
function RunTranscript({ name, runId }: { name: string; runId: string }) {
  ...
  if (!detail.data) {
    return <div className="px-4 pb-3 text-xs text-faintest">Loading transcript…</div>;
  }
  const items = foldRecords(detail.data.events);
  if (items.length === 0) {
    return <div className="px-4 pb-3 text-xs text-faintest">No transcript recorded.</div>;
  }
  return (
    <div className="space-y-1.5 bg-surface-2 px-4 py-3">
      {items.map((it) => {
        switch (it.kind) {
          case 'assistant':
            return (
              <div key={it.key} className="whitespace-pre-wrap break-words text-xs text-body">
                {it.text}
              </div>
            );
          case 'tool':
            return (
              <div key={it.key} className="flex items-center gap-1.5 text-[11px] text-muted">
                <span
                  className={clsx(
          ...
          case 'result':
            return (
              <div key={it.key} className="text-[10px] text-faintest">
...
function RunRow({ name, run }: { name: string; run: AgentRun }) {
  ...
  return (
    <div className="border-b border-line-soft last:border-0">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-2 px-4 py-2 text-left text-xs hover:bg-surface-2"
      >
        <StatusDot status={run.status} />
        <span className="text-muted">{formatDateTime(run.startedAt)}</span>
        <Badge>{run.trigger}</Badge>
        <span className="ml-auto text-faintest">
...
function RunsPanel({ name }: { name: string }) {
  ...
  if (!runs.data) return <div className="border-t border-line-soft px-4 py-3 text-xs text-faintest">Loading…</div>;
  if (runs.data.length === 0) {
    return <div className="border-t border-line-soft px-4 py-3 text-xs text-faintest">No runs yet.</div>;
  }
  return (
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-muted">
            ...
            {agent.nextRun && <span className="text-faintest">next: {formatDateTime(agent.nextRun)}</span>}
          </div>
        </div>
        <button
        ...
        <button
          ...
          className="rounded-lg bg-accent px-3 py-1 text-xs text-white hover:bg-accent-strong disabled:bg-line-strong"
        >
          Run now
        </button>
      </div>
  ...
  return (
    <div className="mx-auto max-w-3xl px-8 py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          onClick={createAgent}
          className="rounded-lg bg-accent px-3 py-1.5 text-sm text-white hover:bg-accent-strong"
        >
          + New agent
        </button>
      </div>
      ...
      {agents.data?.length === 0 && (
        <div className="py-16 text-center text-sm text-faintest">
